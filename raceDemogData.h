...
class raceDemogData {
  public:
        ...
        // raceDemogData(int FN, int A, int B, int L, int HPI, int MR, int W, int WNH, int inother, int size) :
        //     FirstNationCount(FN), AsianCount(A), BlackCount(B), LatinxCount(L), HIPacificIsleCount(HPI),
        //     MultiRaceCount(MR), WhiteCount(W), WhiteNHCount(WNH), otherCount(inother), communityCount(size) {}

        raceDemogData(int FN, int A, int B, int L, int HPI, int MR, int W, int WNH, int size) :
            FirstNationCount(FN), AsianCount(A), BlackCount(B), LatinxCount(L), HIPacificIsleCount(HPI),
            MultiRaceCount(MR), WhiteCount(W), WhiteNHCount(WNH), otherCount(size-FN-A-B-L-HPI-MR-W-WNH), communityCount(size) {}
      ...
      raceDemogData& operator+=(const raceDemogData rd) {
        addFirstNationCount(rd.FirstNationCount);
        addAsianCount(rd.AsianCount);
        addBlackCount(rd.BlackCount);
        addLatinxCount(rd.LatinxCount);
        addHIPacificIsleCount(rd.HIPacificIsleCount);
        addMultiRaceCount(rd.MultiRaceCount);
        addWhiteCount(rd.WhiteCount);
        addWhiteNHCount(rd.WhiteNHCount);
        addOtherCount(rd.otherCount);
